...
for i, im_fn in enumerate(im_fns[:50]):
    ...
    image = cv2.resize(image[80:-80,80:-80,:], (128, 128))
    #print(image.shape)
    ...
    preds = net(image)
    #print(preds)
    preds = net(image).cpu().argmax()
    times_visual.append(time.time() - start_vis)
    #top_preds = np.argsort(preds)[::-1][0:predictions_to_return]
    # output the prediction
    #last_pr = 1 - preds.detach().numpy()
...
plt.savefig(os.path.join(path, "speed_distance_vgg16_gpu_torch.pdf"))
...
plt.savefig(os.path.join(path, "predicts_distance_vgg16_gpu_torch.pdf"))

plt.figure(3, figsize=(10, 4))
plt.plot(times, error)
plt.xlabel("time [s]")
plt.ylabel("error value]")
plt.savefig(os.path.join(path, "error_distance_vgg16_gpu_torch.pdf"))
"""
