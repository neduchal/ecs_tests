...
for i, im_fn in enumerate(im_fns):
    ...
    if len(temperatures) < 40:
        continue
    gt_data.append(c_gt)    
    decision = common.temperature_decision_dirat(temperatures, 1.4)
...
plt.savefig(os.path.join(path, "speed_tempr.pdf"))
...
plt.xlabel("record time [s]")
plt.ylabel("duration [s]")
plt.savefig(os.path.join(path, "speed_tempr_cumulative.pdf"))
...
plt.savefig(os.path.join(path, "predicts_skip_tempr.pdf"))
...
plt.xlabel("time [s]")
#plt.yticks([0,1], ["indoor", "outdoor"])
plt.savefig(os.path.join(path, "triggers_temperature.pdf"))
