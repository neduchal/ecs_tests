...
for i, im_fn in enumerate(im_fns):
    ...
    im = cv2.resize(im[80:-80,80:-80], (128, 128))
    start = time.time()
    times.append((sensors_row[0] - first_timestamp)/(10**9))
...
plt.savefig(os.path.join(path, "predicts_centrist.pdf"))
...
plt.savefig(os.path.join(path, "voting_predicts_centrist.pdf"))
...
plt.savefig(os.path.join(path, "threshold_voting_predicts_centrist.pdf"))

plt.figure(3, figsize=(10, 4))
plt.plot(times, error)
plt.xlabel("time [s]")
plt.ylabel("error value]")
plt.savefig(os.path.join(path, "error_distance.pdf"))
